...
Ext.onReady(function(){
    function editCheck(options){
		...
		if (error)
		{
	        ...
	        if(m.length == 1) document.location.replace('/transaction/receive_order/editro/'+recno);
	...
	store = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: '/transaction/receive_order/json_listing'
		}),
    ...
    var grid = new Ext.grid.EditorGridPanel({
        ...
        tbar: [
        {
        	...
        	handler: function()
        	{
				document.location.replace('/transaction/receive_order/newro');
        	}
        },{
			text: 'Edit RO',
        	iconCls: 'edit',
        	...
        	handler: function()
        	{
				open('/transaction/receive_order/export_excel');
        	}
        },{
			text: 'Print Voucher',
			iconCls: 'print',

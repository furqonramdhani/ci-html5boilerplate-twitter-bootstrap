...
Ext.onReady(function(){
    ...
    var master = new Ext.FormPanel({
		...
		reader: new Ext.data.JsonReader (
		{
			...
			[
				...
				{name: 'nomor_po', type: 'string', mapping: 'pajak'},
        ...
        items: [{
			...
			items:[{
				...
				layout: 'form',
				border:false,
				items:[keterangan,noreff]
			}]
        }]
    });
	var urlHalves = String(document.location).split('/');
    master.getForm().load({url:'/transaction/general_jurnal/get_jv/'+urlHalves[6], waitMsg:'Wait...'});
	...
	store = new Ext.data.Store({
		...
		proxy: new Ext.data.HttpProxy({
			url: '/transaction/general_jurnal/json_listing_dtjv/'+urlHalves[6]
		}),

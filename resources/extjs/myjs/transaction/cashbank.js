Ext.onReady(function(){
    ...
    function editCheck(options){
		...
		if (error)
		{
	        ...
	        if(m.length == 1) document.location.replace('/transaction/cashbank/editcashbank/'+noreff);
	...
	var store = new Ext.data.Store({
		proxy: new Ext.data.HttpProxy({
			url: '/transaction/cashbank/json_listing'
		}),
    ...
    var grid = new Ext.grid.EditorGridPanel({
        store: store,
        cm: cm,
        title: 'Cash/Bank AR (Penerimaan Lainnya)',
        ...
        tbar: [
        {
        	...
        	handler: function()
        	{
				document.location.replace('/transaction/cashbank/newcashbank');
        	}
        },{
			text: 'Edit',
        	iconCls: 'edit',
        	...
        	handler: function()
        	{
				open('/transaction/cashbank/export_excel');
        	}
        }],
